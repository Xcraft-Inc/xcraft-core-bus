...
class Commander extends Router {
  ...
  getRoutingKeyFromId(cmd, id) {
    ...
    if (isCmdAvailable) {
      ...
      if (
        ...
        const {tribe} = appArgs();
        const totalTribes = appArgs()['total-tribes'];

        if (isDistributable) {
          const tokens = id.split('@');
          dstTribe =
            totalTribes > 1
              ? (tokens[tokens.length - 1].charCodeAt(0) % (totalTribes - 1)) +
                1
              : 1;
          if (tribe !== dstTribe) {
  ...
  _onMessage(cmd, msg) {
    ...
    try {
      const routingKey =
        (msg.data && msg.data._goblinRoutingKey) ||
        this.getRoutingKeyFromId(cmd, msg.data && msg.data.id);
      if (routingKey) {
