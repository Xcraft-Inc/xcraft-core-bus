...
var runCommand = function (cmd, msg) {
  ...
  exports.statePush (cmd, msg.orcName);
...
exports.statePop = function () {
  xLog.verb ('try to pop ' + JSON.stringify (state));
  ...
  state.shift ();
  xLog.verb ('poped ' + JSON.stringify (state));
};

exports.statePush = function (cmd, orcName) {
  xLog.verb ('try to push ' + cmd + ' ' + orcName);

  if (state[0].which === orcName) {
    return;
  }

  state.unshift ({
    event: cmd + '.finished',
    which: orcName || 'greathall'
  });
  xLog.verb ('pushed ' + JSON.stringify (state));
};
...
sock.on ('message', function (cmd, msg) {
  ...
  if (commandsRegistry[cmd].activity && exports.getCurrentState ().which !== msg.orcName) {
    xLog.verb ('Creating new activity for ', JSON.stringify (commandsRegistry[cmd]));
    activity.create (cmd, msg, runCommand);
  } else {
    runCommand (cmd, msg);
  }
});
