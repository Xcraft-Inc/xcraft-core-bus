...
class Commander extends Router {
  ...
  #getHorde() {
    if (!this.#xHorde) {
      this.#xHorde = require('xcraft-core-horde');
    }
    return this.#xHorde;
  }
  ...
  #getHost() {
    if (!this.#xHost) {
      this.#xHost = require('xcraft-core-host');
    }
    return this.#xHost;
  }
  ...
  _onMessage(cmd, msg) {
    ...
    try {
      ...
      if (routingKey) {
        ...
        const isPassive = xHorde.getSlave(routingKey).isPassive;
        const isForwarded = !isPassive || isRPC;
        if (isForwarded) {
    ...
    if (
      ...
      xLog.verb(
        () => 'Creating new activity for ' + JSON.stringify(this._registry[cmd])
      );
      activity.execute(
        cmd,
        msg,
  ...
  getFullRegistry() {
    ...
    if (this.isModuleRegistered('horde')) {
      const {appId, appArgs} = this.#getHost();
      const {tribe} = appArgs();
  ...
  registerCommandHandler(name, location, info, rc, handler) {
    xLog.verb(() => `Command "${name}" registered`);
