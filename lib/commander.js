...
class Commander extends Router {
  ...
  _sockSub () {
    this.on ('message', (cmd, msg) => {
      ...
      if (!isCmdAvailable) {
        /* TODO
         * Try to forward the command to the Horde, maybe someone has the
         * handler available. Return an error only if nobody knows the command
         * or if the horde module is not loaded.
         */
        const errorMessage = {};
