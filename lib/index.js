...
class Bus extends EventEmitter {
  ...
  async #loadCommandsRegistry(
    ...
    try {
      await this.loadModule(resp, fileNames, modulePath, {
        name: moduleName,
  ...
  async #loadRegistry(busClient, commandHandlers) {
    const promises = [];
  ...
  async loadModule(resp, files, root, info) {
    ...
    for (const file of files) {
      ...
      if (!Object.hasOwn(handle, 'xcraftCommands')) {
    ...
    for (const item of loaded) {
      if (item.cmds.handlers._postload) {
        await resp.command.sendAsync(`${item.name}._postload`, null);
      }
    }
  }
  ...
  *boot(commandHandlers, next) {
    ...
    xLog.info(() => `Great Hall created: ${genToken}`);
    ...
    this.emit('ready', async (busClient, callback) => {
      try {
        await this.#loadRegistry(busClient, commandHandlers);
