...
const serverConfig = xEtc.load('xcraft-core-server');
class Bus extends EventEmitter {
    ...
    this._commander
      .onInsertOrc((orcName) => {
        ...
        if (orcName.indexOf('@') !== -1) {
          ...
          busClient.events.send('greathall::<axon-orc-added>', orcName);
        }
      })
      .onDeleteOrc((orcName) => {
        const busClient = require('xcraft-core-busclient').getGlobal();
        busClient.events.send('greathall::<axon-orc-removed>', orcName);
      });
  ...
  *loadModule(resp, files, root, info, next) {
    ...
    const isXcraftModule = info.name.startsWith('xcraft-');
    const loaded = [];
    const enableWorkers = serverConfig.enableWorkers;
    for (const file of files) {
      ...
      try {
        ...
        const isWorkerFunc = typeof cmds === 'function';
        if (!isXcraftModule && enableWorkers !== isWorkerFunc) {
          clearRequire(location);
          continue;
        }
        if (isWorkerFunc) {
          xLog.info(`worker loaded: ${name}`);
          cmds = cmds();
        }
      } catch (ex) {
        clearModule(location);
