...
class Bus extends EventEmitter {
  ...
  *loadModule(resp, files, root, info, next) {
    ...
    for (const file of files) {
      let location = path.join(root, file);
      //Avoid require cache division by drive letter casing
      //if the cwd drive letter is lowercase, force location
      //drive letter in lowercase too.
      //
      //root path can randomly arrive with c:\\ or C:\\
      //so we need to normalize manually
      const cwd = process.cwd();
      if (cwd.match(/^[a-z]:\\/)) {
        location = location.replace(/^[A-Z]:\\/, (match) =>
          match.toLowerCase()
        );
      }
